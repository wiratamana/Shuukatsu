...
namespace Tamana.AI
{
    public class AI_Strong : AI_Brain   
    {
        ...
        public void LogicUpdate()
        {
            ...
            else if (ai.distanceFromPlayer > 5.0f)
            {
                ...
                if (isForwardCounterReady && !ai.isUsingSpecialAttack && !ai.isInteruptedByPlayer)
                {
            ...
            else
            {
                ...
                var isPlayerFacingMe = GM.GetAngleBetweenMyForwardAndPlayerForward(ai.transform) > 150;
                if (ai.distanceFromPlayer < 8.5f && attackReader.isPlayerBeginToAttack && !ai.isInteruptedByPlayer && isPlayerFacingMe)
                {
                    if (defensiveMethods[currenIndex] == 0)
                    {
                        PlayDodgeAndCounter();
